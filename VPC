...
vpc_Oct_19 <-read.csv("C:/Users/CP666383/Desktop/Power BI Reports/VPC/vpc rbinding/oct19.csv", stringsAsFactors = FALSE)
...
vpc_Nov_19 <- dbGetQuery(conn, paste("SELECT *
                                     FROM EDWDM.TEDW_SINGCASH_SALESJOURNAL_DOM
                                     WHERE STAT in ('PST','SBM')
...
vpc_Nov_19$TXN_DT <- strptime(as.character(vpc_Nov_19$TXN_DT), "%Y-%m-%d")
vpc_Nov_19$TXN_DT <- format(vpc_Nov_19$TXN_DT, "%Y/%m/%d")

#write.csv(vpc_Nov_19, "C:/Users/CP666383/Desktop/Power BI Reports/VPC/vpc rbinding/nov19.csv", row.names = FALSE)
#Previous Month------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
vpc<- rbind(vpc_Jun_19,firsthalfvpc_Jul_19,secondhalfvpc_Jul_19,vpc_Aug_19,vpc_Sep_19,vpc_Oct_19,vpc_Nov_19)
...
unmappedvpcMerchantData$Merchant[unmappedvpcMerchantData$`Merchant outlets` %like% "FAVEPAY"] <- "Fave by Groupon"
#unmappedvpcMerchantData$Merchant[unmappedvpcMerchantData$`Merchant outlets` %like% "EATSY"] <- "Eatsy"
#unmappedvpcMerchantData$Merchant[unmappedvpcMerchantData$`Merchant outlets` %like% "EATSY"] <- "Eatsy"
#unmappedvpcMerchantData$Merchant[unmappedvpcMerchantData$`Merchant outlets` %like% "EATSY"] <- "Eatsy"
#unmappedvpcMerchantData$Merchant[unmappedvpcMerchantData$`Merchant outlets` %like% "EATSY"] <- "Eatsy"
...
viavisualisation <- dbGetQuery(conn, paste("SELECT *
                                      FROM EDWDM.TEDW_SINGCASH_SALESJOURNAL_DOM
                                           WHERE STAT IN ('PST')
                                           AND TXN_DT BETWEEN to_date('01-Jan-2019','DD-Mon-YY') AND to_date('01-Dec-2019','DD-Mon-YY')
                                           AND orig_amt > 0
